...
exports.vaultService = exports.ObsidianVaultService = void 0;
...
class ObsidianVaultService {
    ...
    constructor() {
        ...
        this.vaultPath = path_1.default.join(process.cwd(), 'docs', 'vault');
        if (!fs_1.default.existsSync(this.vaultPath)) {
            fs_1.default.mkdirSync(this.vaultPath, { recursive: true });
            (0, LogService_1.pushLog)(`🗄️ Obsidian Vault 初始化完成: ${this.vaultPath}`, 'info');
        }
    }
    /**
     * 將當前任務的語義圖譜匯出為 Obsidian 格式
     */
    exportGraphToVault(missionTopic) {
        const { nodes, links } = VectorService_1.vectorService.getGraphData();
        if (nodes.length === 0)
            return;
        ...
        nodes.forEach(node => {
            ...
            fs_1.default.writeFileSync(path_1.default.join(this.vaultPath, nodeFileName), nodeContent, 'utf-8');
        });
        // 寫入 MOC
        fs_1.default.writeFileSync(path_1.default.join(this.vaultPath, mocFileName), mocContent, 'utf-8');
        (0, LogService_1.pushLog)(`📥 知識沉澱完成：已將 ${nodes.length} 個節點實體化至 Obsidian Vault。`, 'success');
        // 重置圖譜準備下一次任務
        VectorService_1.vectorService.clear();
    }
}
exports.ObsidianVaultService = ObsidianVaultService;
exports.vaultService = new ObsidianVaultService();
