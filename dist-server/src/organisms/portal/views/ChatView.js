...
exports.renderChatView = void 0;
const m_portal_hud_1 = require("../../../molecules/m_portal_hud");
const MessageRenderer_1 = require("./chat/MessageRenderer");
const renderChatView = (messages, pollingCycles, currentPasses, filterRound, isStreaming, activePrompt) => {
    const win = window;
    const harness = win.harnessState || (win.semanticGraph?.context?.harnessState);
    let harnessHtml = '';
    if (harness) {
        const facts = (harness.facts || []).map((f) => `
        ...
        const vars = Object.entries(harness.variables || {}).map(([k, v]) => `
    ...
    return `
        <div class="h-full flex flex-col relative overflow-hidden bg-transparent">
            ...
            <div class="h-[60px] border-b border-white/5 flex items-center bg-black/80 backdrop-blur-3xl px-4 shrink-0 z-50">
                <div class="flex items-center gap-6 shadow-2xl">
                    ${(0, m_portal_hud_1.renderLiveTelemetry)(currentPasses, pollingCycles)}
                    ${(0, m_portal_hud_1.renderRoundFilters)(pollingCycles, currentPasses, filterRound)}
                    ${(0, m_portal_hud_1.renderCycleController)(pollingCycles)}
                </div>
        ...
        .map(m => (0, MessageRenderer_1.renderMessageBlock)(m)).join('')}
            </div>
        </div>`;
};
exports.renderChatView = renderChatView;
