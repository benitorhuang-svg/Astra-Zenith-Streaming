...
exports.renderWelcomeView = void 0;
 ...
 */
const renderWelcomeView = (error = '', serverAuthorized = false) => {
    const identity = identity_1.GlobalIdentity.get();
    return `
...
exports.renderWelcomeView = renderWelcomeView;
/**
 * ORGANISM: IDENTIFICATION_BOARD
 */
const renderIdentificationBoard = (error, authorized, identity) => `
...
`;
const renderOperatorIDInput = (identity) => `
...
`;
const renderCredentialInput = (error, authorized, identity) => {
    if (authorized && !error) {
        return `
            <div class="flex flex-col gap-2 group/input">
    ...
    return `
        <div class="flex flex-col gap-1">
            ...
            <az-input placeholder="SECURE_PHRASE..." value="${identity.apiKey}" icon="lock" id-attr="u-id-key" type="${window._PASSWORD_VISIBLE ? 'text' : 'password'}" ${error ? 'error' : ''}>
                <button slot="right" id="u-btn-toggle-password" class="text-white/20 hover:text-white/60 transition-colors cursor-pointer flex items-center justify-center">
                     <i data-lucide="${window._PASSWORD_VISIBLE ? 'eye-off' : 'eye'}" class="w-4 h-4 pointer-events-none"></i>
                </button>
            </az-input>
