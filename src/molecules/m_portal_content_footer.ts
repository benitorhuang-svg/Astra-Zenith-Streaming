...
export interface FooterOptions {
    height?: string; // Default: h-[48px]
    left?: string;
    center?: string;
    right?: string;
    containerClass?: string;
    bgClass?: string; // Default: bg-black/40 backdrop-blur-xl
}

export const renderCommonFooter = (opts: FooterOptions = {}) => {
    const {
        height = 'h-[48px]',
        left = '',
        center = '',
        right = '',
        containerClass = '',
        bgClass = 'bg-black/40 backdrop-blur-xl'
    } = opts;
