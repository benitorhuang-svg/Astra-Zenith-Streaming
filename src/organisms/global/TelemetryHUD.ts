...
export class TelemetryHUD extends HTMLElement {
    ...
    public updateStats(rpm: number, rpmLimit: number, rpd: number, rpdLimit: number) {
        const rpmPct = Math.min((rpm / rpmLimit) * 100, 100);
        const rpdPct = Math.min((rpd / rpdLimit) * 100, 100);

        const rpmBar = this.querySelector('#u-hud-rpm-bar') as HTMLElement;
        const rpdBar = this.querySelector('#u-hud-rpd-bar') as HTMLElement;
        const rpmVal = this.querySelector('#u-hud-rpm-val') as HTMLElement;
        const rpdVal = this.querySelector('#u-hud-rpd-val') as HTMLElement;
        ...
        if (rpdBar) rpdBar.style.width = `${rpdPct}%`;
        if (rpmVal) rpmVal.textContent = `${rpm} / ${rpmLimit}`;
        if (rpdVal) rpdVal.textContent = `${rpd} / ${rpdLimit}`;
    }
