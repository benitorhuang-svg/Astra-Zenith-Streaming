...
export function renderAgentInfoBubble(agentCode: string, activePrompt: string, selectedAgentPromptContent: string | null, apiKey: string, agentModel: string): string {
    ...
    const isFreeTier = !apiKey || apiKey.toLowerCase() === 'free';
    ...
    const specializedModels = [
        ...
        { id: "gemma-4-it", name: "Open Gemma 4" }
    ];
            ...
            <div class="relative flex-1 bg-black/98 border border-primary/30 rounded-t-xs shadow-[0_0_80px_rgba(var(--primary-rgb),0.25)] flex overflow-hidden">
                ...
                <div class="w-80 border-r border-white/10 flex flex-col p-8 bg-linear-to-b from-primary/10 to-transparent shrink-0">
                    <div class="relative aspect-square w-full border border-primary/40 p-1.5 bg-black/60 group overflow-hidden shadow-inner mb-6">
                        ...
                        <!-- Tech Brackets -->
                        <div class="absolute top-0 left-0 w-3 h-3 border-t-2 border-l-2 border-primary/60"></div>
                        <div class="absolute top-0 right-0 w-3 h-3 border-t-2 border-r-2 border-primary/60"></div>
                        <div class="absolute bottom-0 left-0 w-3 h-3 border-b-2 border-l-2 border-primary/60"></div>
                        <div class="absolute bottom-0 right-0 w-3 h-3 border-b-2 border-r-2 border-primary/60"></div>
                    </div>
                    ...
                    <div class="flex flex-col gap-6">
                        <div class="flex flex-col gap-2">
                            ...
                            <div class="relative group">
                                <select id="u-model-selector" class="w-full h-10 bg-white/5 border border-white/10 px-3 text-[11px] font-mono text-primary font-bold uppercase tracking-wider appearance-none cursor-pointer focus:border-primary/50 outline-none hover:bg-white/10 transition-all text-center">
                                    ${!isFreeTier ? `
                                    <optgroup label="💎 PAID FRONTIER CORES" class="bg-[#0a0f18] text-primary/60">
                                        ${paidModels.map(m => `<option value="${m.id}" ${m.id === currentModel ? 'selected' : ''} class="bg-[#0a0f18] text-white">${m.name}</option>`).join('')}
                                    </optgroup>
                                    ` : `
                                    <optgroup label="💎 PAID CORES (LOCKED)" class="bg-[#0a0f18] text-white/20">
                                        <option disabled class="bg-[#0a0f18] text-white/20">Upgrade to Paid for Pro Models</option>
                                    </optgroup>
                                    `}
                        ...
                        <!-- Apply Button -->
                        <button id="u-btn-bubble-apply" class="h-10 px-4 bg-primary/20 border border-primary/40 text-primary font-mono text-[10px] font-black uppercase tracking-[0.2em] hover:bg-primary hover:text-black transition-all active:scale-95 shadow-[0_0_15px_rgba(var(--primary-rgb),0.2)]">
                            Apply_Tactical_Update
                        </button>
                    </div>
                ...
                <div class="flex-1 flex flex-col relative min-w-0">
                    ...
                    <div class="p-10 pb-4 flex flex-col h-full overflow-hidden">
                        ...
                        <div class="flex items-center gap-4 mb-8">
                            <button id="u-btn-bubble-reset" class="h-8 px-4 bg-white/5 border border-white/10 text-white/40 font-mono text-[9px] font-black uppercase tracking-[0.15em] hover:text-white hover:bg-white/10 transition-all active:scale-95 flex items-center gap-2">
